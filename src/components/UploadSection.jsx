...
export default function UploadSection() {
  ...
  const handleMockUpload = () => {
    ...
    setTimeout(() => {
      setAiData({
        rooms: {
          bedroom1: ["Bed", "Wardrobe", "2x Nightstands"],
          kitchen: ["Fridge", "Oven", "Microwave", "Cabinets (contents estimated)"]
        },
        supplies: {
          smallBoxes: 25,
          largeBoxes: 18,
  ...
  const handleDownload = () => {
    ...
    if (aiData) {
      ...
      let y = 38;
      Object.entries(aiData.rooms).forEach(([room, items]) => {
        doc.text(`- ${room}: ${items.join(", ")}`, 30, y);
        y += 8;
      });
  ...
  return (
    <section className="py-12 px-4 md:px-8 bg-gray-50">
      <div className="max-w-5xl mx-auto text-center">
        <h2 className="text-3xl font-bold mb-4">AI Analysis & CRM Integration</h2>
        <p className="text-gray-600 mb-6">
          Upload a video, image or PDF. Our AI detects furniture, appliances and estimates supplies automatically.
        </p>

        <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
          <Button onClick={handleMockUpload} className="w-full sm:w-auto">
            ...
            <Card className="bg-white shadow-md rounded-2xl p-4">
              <CardContent>
                ...
                <ul className="text-left text-sm text-gray-700 list-disc list-inside">
                  {Object.entries(aiData.rooms).map(([room, items], idx) => (
                    <li key={idx}>{room}: {items.join(", ")}</li>
                  ))}
                </ul>
